...
`switch` 语句中的 `case lable` 要求必须不同，但是并不意味着不同的 `lable` 需要执行不同的的操作。几个 `case lable` 执行相同的操作也是常见的状态。下面的示例程序用于判断是否是元音字母 
...
很多时候，这种贯穿都是因为疏忽大意而缺少 `break` 导致的。然而，并不是所有的贯穿都是 BUG，上一个示例程序就是利用的这种贯穿的特性。为了说明代码中故意使用贯穿的特性，C23 标准加入了特殊属性 `[[fallthrough]]` 用于说明此处需要利用贯穿特性。因此，上一个示例程序应该修改为
