...
要编写这个程序，需要一个文本编辑器或 **集成开发环境(Integrated Development Enviroment, IDE)**。可供选择的工具有很多。但是，目前，先打开我们最熟悉的文本编辑软件，然后输入下面给出的例程，最后将保存文件并为 [`hello.c`](../src/ch01/hello.c)
...
`printf()` 中的转换说明符 `%6.2f` 说明打印浮点数时至少占 $6$ 个字符宽度，`%.2f` 指定待打印浮点数的小数点后有 $2$ 位小数，但是没有宽度限制。如果省略宽度和精度，即 `%f` 仅仅要求打印浮点数。下表列出了一些常用的转换说明符
