...
决策是程序设计语言的基本功能。任何一种程序设计语言都必须提供根据不同条件选择不同指令执行的功能。
...
下面的示例程序 [`ex03_01.c`](../src/ch03/ex03_01.c) 演示了比较运算符的工作方式
...
![1772857076374.png](https://fastly.jsdelivr.net/gh/duyupeng36/images@master/2026-03/1772857076415.png)
...
下面的示例程序 [`ex03_02.c`](../src/ch03/ex03_02.c) 演示了 `if` 语句，这个示例程序检查从键盘上输入的一个整数值
...
当表达式 `expression` 的值为 `false` 时，就执行 `else` 后边的语句。下面的示例代码，演示了 `if-else` 语句的使用，这个示例代码 [`ex03_03.c`](../src/ch03/ex03_03.c) 从键盘中读入一个整数，判断该整数是否是偶数
...
下面的示例程序 [`ex03_04.c`](../src/ch03/ex03_04.c) 演示一个完整的 `if` 语句用法，该示例程序用于计算股票经纪人的佣金。投资者通过股票经纪人进行股票交易时，会按照下表列出的交易额给付给股票经纪人佣金
...
在有两个或更多相关条件的地方使用 `if` 语句较为繁杂。例如，搜索年龄在 $21 \sim 35$ 之间、拥有学士或硕士学历、未婚的女性。在使用 `if` 语句是就需要层层嵌套下去。这会导致 `if` 语句嵌套得非常深。

**逻辑运算符** 则提供了一种简洁地解决方案。使用逻辑运算符，可以把一系列的比较组合到一个表达式中，这样无论条件多么复杂，都可以使用一个 `if` 语句解决。下表列出了 C 语言提供了 $3$ 个逻辑运算符
...
下面的示例程序 [`ex03_05.c`](../src/ch03/ex03_05.c) 演示了条件表达式和逻辑运算符组在一起的使用场景。该示例程序用于确定一个人是否有贷款风险
...
条件运算符也称为 **三元运算符**，因为它涉及三个操作数，这也是 C 语言中唯一的三元运算符。它是 `if-else` 语句的简化版本。例如，比较两个数，获取其中最大的数，使用 `if-else` 语句完成如下
...
有时候，我们常常需要把表达式和一系列值进行比较。C 语言提供的 `switch` 语句可以从多个选项中选择。选项使用整型常量表达式标识，称为标记(`lable`)，根据给定的整数类型表达式的值确定选择哪个 `lable` 的代码执行。

下面是 `switch` 语句最常用的格式如下
...
在 C 语言中，`switch` 语句要求 `expression` 必须是整型表达式，并且每个分支的 `lable` 必须是整型常量表达式。请注意，**每个分支标号的后边可以跟任意数量的语句**，并且不需要使用花括号。每组语句的最后一条通常是 `break` 语句

> [!warning]
>
> 请注意，C 标准不允许有重复的 `lable` 出现，但是不要求每个分支的顺序。也就是说，`defalut` 分支可能出现在任意顺序位置。

`switch` 语句执行时，首先计算 `expression` 表达式的值，然后从分支中选择 `expression == lable` 的分支执行。如果没有找到对应的 `lable`，则执行 `default` 分支

> [!tip]
>
> 注意 `defalut` 分支是可选的。如果 `switch` 语句中没有 `default` 分支，并且没有与 `expression` 匹配的 `lable`，则 `switch` 语句什么也不会做。

下面的示例程序 [`ex03_06.c`](../src/ch03/ex03_06.c) 演示了 `switch` 语句的用法
...
`switch` 语句中的 `case lable` 要求必须不同，但是并不意味着不同的 `lable` 需要执行不同的的操作。几个 `case lable` 执行相同的操作也是常见的状态。下面的示例程序 [`ex03_07.c`](../src/ch03/ex03_07.c)  用于判断是否是元音字母
...
每组 `case` 语句最后的 `break` 语句使得 `switch` 之后的语句开始执行。如果 `case` 语句最后没有 `break` 语句，那么就会继续执行后一个 `case` 语句。这种现象称为 **贯穿**(fallthrough)。

很多时候，这种贯穿都是因为疏忽大意而缺少 `break` 导致的。然而，并不是所有的贯穿都是 BUG，上一个示例程序就是利用的这种贯穿的特性。为了说明代码中故意使用贯穿的特性，C23 标准加入了特殊属性 `[[fallthrough]]` 用于说明此处需要利用贯穿特性。因此，上一个示例程序应该进行如下修改，得到 [ex03_08.c](../src/ch03/ex03_08.c)
...
`switch` 语句一般在花括号中包含自己的语句块，其中包括 `case` 语句。`if` 语句也常在花括号中包含语句块，无论条件为 `true` 还是 `false` 时执行。

> [!warning]
>
> 请注意，`if` 语句的语句和 `switch` 语句的语句块与定义变量作用域时涉及的其他语句块没有任何区别
>
> 在语句块中声明的任何变量的作用域只在该语句块内部，所以不能在语句块外引用他们
...
printf("savit = %s", savit);  // ERROR: savit 不存在
```

对于 `if` 语句块来说，这个规则与其他语句块规则完全一致。然而，对于 `switch` 语句块中定义的变量必须在执行该语句块时能够进行访问，不能通过跳转到相同作用域内位于变量声明之后的 `case` 子句来绕过定义。通过一个示例代码片段来理解这个规则将更显而易见
...
switch (test)
{
    int i = 1;  // 非法的: 不能执行
    case 1:
        int j = 2;  // 非法的：可以执行但是可以绕过它的定义进入其作用域
        printf("test + j = %d\n", test + j);
        break;
    ...
    case 3:
    {
        int m = 4;  // OK：可以执行并且不能绕过它的定义进入其作用域
        printf("test + m = %d\n", test + m);
        break;
    }
    default:
        int n = 5;  // ok: 可以执行并且不能绕过它的定义进入其作用域
        printf("test + n = %d\n", test + n);
        break;
}

// 下面两条语句都是非法的，j 和 n 已经不存在了
printf("j = %d\n", j);
printf("n = %d\n", n);
```
...
> [!tip]
>
> 在 `switch` 语句中，合法的变量定义必须可以在该语句块执行过程中访问；并且不能绕过该变量的定义进入该变量的作用域
